...
def setup_problem(n, mms, params):
    ...
    wall = CompiledSubDomain('near(x[0]*(1-x[0])*x[1]*(1-x[1]), 0)')
    wall.mark(surfaces, 2)
    
    # File('surface.pvd') << surfaces
    ...
    a[1][1] = avg_area*inner(grad(u1), grad(v1))*dx + avg_area*inner(u1, v1)*dx
    # Coupling
    ...
    # Homog everywhere
    V3_bcs = [DirichletBC(V3, g3, surfaces, 2)]
    V1_bcs = []
    ...
    # print np.min(np.abs(np.linalg.eigvalsh(ii_convert(A).array())))
...
def setup_mms(params):
    ...
    u1d = Expression('cos(pi*x[2])', degree=4)
    f1d = Expression('(1+pi*pi)*cos(pi*x[2])', degree=4)

    p = Expression('0', degree=4)  # The multiplier
    fLM = Expression('sin(2*pi*x[0])*sin(2*pi*x[1]) - cos(pi*x[2])', degree=4)
