...
superconvergent. We speculate that the result is (i) due to the fact that the
exact solution is particularly simple, $L=0$, and (ii) the fact that the error
is interpolated into the same finite element as the approximation $L_h$. We remark
that for $u$ and $\ud$ the error is interpolated into FE space of piecewise
quadratic \emph{discontinous} functions. Evaluating the fractional norm in higher
order discontinous spaces is prohibitively costly. \textbf{Should we try for line formulation
where  this is okay?}
%
\begin{table}
  \scriptsize{
...
\hline
  ...
  \scriptsize{%(ii)
    ...
    $h$ & $\norm{u-u_h}_{1, \Omega}$ & $\norm{\ud-\udh}_{1, \Lambda}$ & $\norm{L-L_H}_{-1/2, \Lambda}$\\
      \hline
4.3E-1 & 3.1E0(--) & 5.4E-1(--) & 4.3E-2(--)\\
2.2E-1 & 1.7E0(0.87) & 2.6E-1(1.06) & 1.1E-2(1.99)\\
1.1E-1 & 8.6E-1(0.96) & 1.3E-1(1.02) & 2.7E-3(2.01)\\
5.4E-2 & 4.4E-1(0.99) & 6.3E-2(1.00) & 6.7E-4(2.00)\\
...
time for \eqref{eq:problem1_simple} is up to 2 times longer compared to
...
the multiplier space as small as possible.
