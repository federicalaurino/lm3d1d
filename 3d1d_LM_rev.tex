...
% old commands: used for changes before review
...
\miro{We remark that $\lambda$ can be viewed as a Lagrange multiplier associated with the
  coupling constraint \eqref{eq:coupling}, see \S\ref{sec:derivation} for
...
Let $\Sigma$ be a Lipschitz co-dimension one subset of $D$. We denote with $\mathcal{T}_\Sigma: H^q(D) \rightarrow H^{q-\frac 1 2}(\Sigma)$ the trace operator from \kentold{$D$} to $\Sigma$.
...
space \miroold{$H^s(\Gamma; w)$} is defined in terms of the interpolation of the corresponding weighted spaces. 
...
\end{equation*}
whereas a weighted fractional
space \miroold{$H_{00}^s(\Gamma; w)$} is defined in terms of the interpolation
of the corresponding weighted spaces (see \cite[ch. 2.1]{MR0350177}, \cite{chandler2015interpolation}). 
For the norm of such spaces, we introduce 
the Riesz map $S$ such that 
for $u, v \in H_0^1(\Gamma)$ we have 
\[
\int_{\Omega} w \nabla\left(S u\right)\cdot\nabla vd\omega  = (u,v)_{L^2(\Gamma), w}.
\]
Then $S=-\Delta^{-1}$ is a compact self-adjoint operator. 
Assuming that $\{\lambda_k\}_k$ is the set of eigenvalues, 
$\{\phi_k\}_k$ the set of eigenvectors of $S$ 
orthonormal with respect to the inner 
product $(\cdot, \cdot)_{L^2(\Gamma), w}$ and $u\in H_0^1(\Gamma)$ can be expressed 
as $u=\sum_k c_k \phi_k$ 
then
\[
\|u\|^2_{H_{00}^s(\Gamma), w} = \sum_k \lambda_k^{-s} c_k^2 .   
\]
%The space $H_{00}^s(\Gamma; w)$ is defined analogously, but with $S$ above defined in terms of the $H^1_0$ inner product.
Owing to the positivity and boundedness of $w$ the weighted spaces equal the corresponding non-weighted spaces as sets,
...
If $u\in H^{\frac 12}_{00}(\Gamma)$ then $\avrc{u}\in H^{\frac 12}_{00}(\Lambda)$ and there exists a constant $C_\Gamma$, \paoloold{bounded independently of $\D$ and $\DD$, such that}
\begin{equation*}
...
which is a consequence of the definition of $\mtrace$, Jensen inequality, \kentold{and the fact that the patches are disjoint}
...
\end{multline}
%\kentold{Earlier in the paper we always specified the measure, ds, dx etc}
...
\kentold{exploiting Jensen's average inequality} \eqref{eq:avrg_ineq} 
...
\paoloold{(where all the constants have been condensed into $C$),}
\begin{multline*}
...
For the second assumption of Lemma \ref{lemma23burman}, we recall that $b(v_h,\mdh)$ is continuous \kentold{with respect to the norms} $\vertiii{v_h},\,\|\mdh\|_{L^2(\Lambda)}$. 
...
(the proof is \miroold{analogous} to the one of Lemma 2.1 in \cite{burman2014})
...
% \end{multline*}
% \kentold{Is it $C_P$}
% \paoloold{For the third term we observe that since $H \lesssim 1$, there exists a constant $C$ 
...
% \up &= \paoloold{u_b} && \text{ on } \partial \Omega,
% \end{align}
% \end{subequations}
...
$\Lambda$ and $\Gamma$ conforming discretization of $\Omega$
   ...
   Each cell of $\mathcal{T}^{\Gamma}_h$ (in blue, filled marker vertices) and $\mathcal{T}^{\Lambda}_h$ (in red, filled marker vertices)
   is a facet, respectively edge, of $\mathcal{T}^{\Omega}_h$ (in black, empty square marker vertices).
