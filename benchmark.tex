...
Using benchmark problem \eqref{benchmark} we finally consider \eqref{eq:problem2} in the
...
lying on $\Gamma$. Further we let $h^{\prime}=h/3$ in $\mathcal{T}^{\Gamma}_{h^{\prime}}$,
...
\hline
  ...
  \captionof{table}{
    ...
    $\mathcal{T}^{\Omega}_h$ does not conform to $\Gamma$.}
  \label{tab:unfit}    
  \end{minipage}
\end{table}

Using discretization in terms of $P_1$-$P_1$-$P_0$ element Table \ref{tab:unfit}
lists the error of the stabilized formulation \eqref{eq:problem2}. A linear
convergence in the $H^1$ norm can be observed in the error of $u$ and $\ud$.
For simplicity the convergence of the multiplier is measured in the $L^2$ norm
